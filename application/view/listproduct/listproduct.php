    			...
    			if(result.flag == true){	
    				...
    				for (var j = 0; j< ids1.length; j++) {
    					if(ids1[j].checked){
    		    			...
    		    			$('#listproductbody .checkbox[data-checkbox="'+res[1]+'"]').val(1+";"+res[1]);
    		    			$('#listproductbody .checkbox[data-checkbox="'+res[1]+'"]').prop("checked", false);
    		    			$('#listproductbody .status[data-status="'+res[1]+'"]').html("<?=$this->language("l_status3")?>");
    					}
    				}
    			}
    		}, true);
	...
	$('#checklist').on('click', function(){
		...
		$($.elt_popup).on('click', '.checklist #btnchecklist', function(){	
            ...
            if (email1 != "" || email2 != "" || email3 != "") {
            	$.fn_ajax('sendmailChecklist', {'email1':email1,'email2':email2,'email3':email3,'checkproduct':checkproduct}, function(result){ 
            		console.log(result);
            		$($.elt_popup).hide().find('.absolute').hide();
            		var ids1=document.getElementsByName("checkproduct[]");
                        for (var j = 0; j< ids1.length; j++) {
...
<div class="ct_content">
	<div class="form_group">
		<form class="search_member" action="<?=$this->route('listproduct', ['method'=>'sale'])?>" method="POST" name="mform" id="mform">
			<table class="table" >
				<tr>
					<th><?=$this->language('l_listproduct22')?></th>
					<td colspan="3">
						<select name="m_keytime" class="select small">
							<option <?php if ($m_keytime==0) echo 'selected="selected"';?> value="0"><?=$this->language('l_listproduct3')?></option>
							<option <?php if ($m_keytime==1) echo 'selected="selected"';?> value="1"><?=$this->language('l_listproduct4')?></option>	
						</select>
		...
		<div class="table_head bgwhite">
			<div class="ctrl_head clearfix align-left">
				...
				<div class="icons col-xs-7 align-right">
					...
					<button type="button" name="" class="btn hover checklist" id="checklist"><?=$this->language('l_listproduct21')?></button>
