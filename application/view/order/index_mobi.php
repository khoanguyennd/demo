...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('#confirmuse').on('click', function () {
            var mess1 = '개의 티켓 중 구매완료 티켓';
            var mess2 = '개를 사용처리하시겠습니까?';
            var data = $.fn_dataChecked(1);
            if (data.formdata && data.formdata.length > 0) {
                if (data.result == "checkallbuy") {
                    if ($(this).attr('data-action') == 'true' && $($.elt_popup + ' .confirm').is(':visible')) {
                        $.fn_confirm(false);
                        $.fn_changestatus(data.formdata);
                    ...
                    if ($(this).attr('data-action') == 'true' && $($.elt_popup + ' .confirm').is(':visible')) {
                        ...
                        $.fn_confirm(true, {'class': 'changestatus', 'data-action': 'confirmuse'}, res[2] + " " + mess1 + " " + res[1] + " " + mess2);
                        $($.elt_popup).find('.back').val("취소");
                        $($.elt_popup).find('.continue').val("확인");
                        $(this).attr('data-action', 'true');
                    }
                ...
                if (data.result == "nocheck") {
                    $.fn_alert(true, true, '<?= $this->language('l_listproduct7') ?>');
                }
                if (data.result == "pleasecheck") {
                    $.fn_alert(true, true, '<?= $this->language('l_orderwarning1') ?>');
                }
            }
        });

        $.fn_dataChecked = function (status) {
            var formdata = [];
            var result = "";
            ...
            if ($(elemnt).length > 0) {
                ...
                if (temp > 0) {
                    ...
                    if (temp == total) {
                        ...
                        result = "checknoallbuy#" + temp + "#" + total;
                    }
                } else {
                    formdata = [];
                    result = "pleasecheck";
            ...
            var data = {'formdata': formdata, 'result': result};
            return data;
        }

        // Thực hiện khôi phục
        $('#confirmrestore').on('click', function () {
            var mess1 = '개의 티켓 중 사용완료 or 환불요청중 티켓';
            var mess2 = '개를 복원처리하시겠습니까?';
            var data = $.fn_dataChecked(2);
            if (data.formdata && data.formdata.length > 0) {
                if (data.result == "checkallbuy") {
                    if ($(this).attr('data-action') == 'true' && $($.elt_popup + ' .confirm').is(':visible')) {
                        $.fn_confirm(false);
                    ...
                    if ($(this).attr('data-action') == 'true' && $($.elt_popup + ' .confirm').is(':visible')) {
                        ...
                        var res = data.result.split("#");
                        $.fn_confirm(true, {'class': 'changestatus', 'data-action': 'confirmrestore'}, res[2] + " " + mess1 + " " + res[1] + " " + mess2);
                        $($.elt_popup).find('.back').val("취소");
                        $($.elt_popup).find('.continue').val("확인");
                        $(this).attr('data-action', 'true');
                    }
                ...
                if (data.result == "pleasecheck") {
                    $.fn_alert(true, true, '<?= $this->language('l_orderwarning2') ?>');
                }
            }
        });
            ...
            $.fn_ajax('changeStatus', {'status': data}, function (result) {
                if (result.flag == true) {
                    $.each(result.id, function (index, value) {
                        ...
                        var position = url.lastIndexOf('manage-order-sale.html');
                        if (position != -1) {
                            ...
                            $(element).find('td span.rows_order_span').text(result.status);
                        } else {
                            var element = '#tableorder tbody tr[data-id="' + value + '"]';
                            $(element).remove();
                            $('#' + value).remove();
                            $('#total_result').html(result.spantotal);
                        }
                    });
                }
            }, true);
            ...
            <div class="t_search_qldh">
                <form action="<?= $this->route('order', ['method' => 'sale']) ?>" method="POST" id="nf" name="nf">
                    ...
                    <div id="myModal" class="modal">
                        <div class="modal-content" style="max-height: 95%;overflow: auto;">
                    ...
                    <!-- end The Modal -->
                    <img id="reset_text_search" style="<?php if ($text_ten_phone_mave == "") {
                                    echo 'display:none;';
                                } else {
                                    echo 'display:block;';
                                } ?>" src="<?= URL_PUBLIC ?>temple_mobile/img/reset_text.png" alt=""/>
        ...
        <div class="table_head bgwhite" style="padding:215px 0px 10px 0px">
            <div class="ctrl_head clearfix" style="position: fixed;background: #fff;width: 97.5%;z-index: 3;">
                <div class="title t_action">
                    <p class="p_t_action_1"><?= $this->language('l_mbtotal') ?><span id="total_result"><?= $this->getData('total') ?></span></p>
                    <p class="p_t_action_2"> 
                            <?php if ($this->_params['method'] == "sale") { ?>
                            <button style="padding: 6px 5px 8px 5px; border: 1px solid #a9a9a9; font-weight:bold;" type="button" id="checkall">
                                <input style="display:none;" type="checkbox" name="checkbox" id="tbcheckall" value="all">
