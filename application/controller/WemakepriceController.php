...
class WemakepriceController extends Controller {
    ...
    private function insertProductWMP($token, $dataInsert) {
    ...
    private function updateProductWMP($token, $goodIds, $data, $step) {
        ...
        if ($step == 1) {
            ...
            $sql = '    SELECT sc.sellerProductId, sc.travelProductId, st.sellerTravelItemId, st.travelItemId, st.optionCategoryId, tv.name, 
                                tv.isDelete, sc.type_rate, tv.onsale, ra.price, ra.pricesale, ra.amount, ra.saleEndedAt
                        FROM tb_salechannel sc, tb_salechannelitem st, tb_travelitems tv, tb_rates ra
                        WHERE sc.sellerProductId = st.sellerProductId AND st.sellerTravelItemId = tv.sellerTravelItemId 
                                AND tv.sellerTravelItemId = ra.sellerTravelItemId AND sc.travelProductId = "' . $goodIds . '" ';

            $this->_model->setQuery($sql);
            $results3 = $this->_model->readAll();

            $optionName = [];
            foreach ($results3 as $items) {
                array_push($optionName, $items['name']);
            }

            $data = ["categoryInfos" => [
                    [
                        "categoryName" => "option name",
                        "categoryValues" => $optionName,
                        "depth" => 1
                    ]
                ],
                "optionBase" => [
                    "basicPrice" => (float) $results3[0]['price'],
                    "salePrice" => (float) $results3[0]['price'], //pricesale
                    "stockQuantity" => $results3[0]['amount'],
                    "isDisplay" => ($results3[0]['onsale'] == 1) ? true : false
                ],
            ...
            foreach ($objOptions['optionCategoryMap'] as $item) {
                $optionCategoryId[$i] = $item['optionCategoryId'];
                $i++;
            }
            ...
            $editOptions = 0;
            foreach ($results3 as $items) {
                if (empty($items['travelItemId'])) {
                    $addOptions = 1;
                    $onSale = true;
                    if ($items['isDelete'] == 1) {
                        $onSale = false;
                    } elseif ($items['onsale'] == 0) {
                        $onSale = false;
                    }
                    $optionData = [
                        'basicPrice' => $items['price'],
                        'displaySaleEndDate' => null,
                        'isDisplay' => $onSale,
                        'optionCategoryId' => $optionCategoryId[$i],
                        'optionCategoryValues' => (string) $items['name'],
                        'partnerGoodsCode' => null,
                        'rateValue' => $items['type_rate'],
                        'saleEndDate' => null,
                        'salePrice' => $items['price'], //pricesale
                        'stockQuantity' => $items['amount'],
                        'useDate' => null
                    ];
                    ...
                    $optionData = [
                        'basicPrice' => $items['price'],
                        'id' => (int) $items['travelItemId'],
                        'isDisplay' => $onSale,
                        'optionCategoryId' => $items['optionCategoryId'], //$optionCategoryId[$i],
                        'optionCategoryValues' => (string) $items['name'],
                        'partnerGoodsCode' => null,
                        'rateValue' => $items['type_rate'],
                        'saleEndDate' => '2020-07-31T00:30:00', //$items['saleEndedAt'],
                        'salePrice' => $items['price'], //pricesale
                        'stockQuantity' => $items['amount'],
                        'useDate' => null
                    ];
            ...
            if ($options) {
                $data = [
                    'optionBaseId' => $objOptions['optionBase']['id'],
                    "options" => $options
                ];
                ...
                $urlOptionsId = 'https://tour-partner-api.wonders.app/v1/partner/activity/products/' . $goodIds . '/options/search';
                $objOptionsIds = $this->callAPI("POST", $urlOptionsId, json_encode($data), $token);

                $travelItemId = [];
                $i = 0;
                foreach ($objOptionsIds['results'] as $iemOptionIdValue) {
                    $travelItemId[$i] = $iemOptionIdValue['id'];
                    $i ++;
                }
                ...
                foreach ($results3 as $items) {
                    $data = [
                        'travelItemId' => isset($travelItemId[$i]) ? $travelItemId[$i] : '',
                        'optionCategoryId' => isset($optionCategoryId[$i]) ? $optionCategoryId[$i] : ''
                    ];
                    $i ++;
                    $this->_model->updateRecord('salechannelitem', $data, ['sellerTravelItemId' => $items['sellerTravelItemId']]);
                }
            }
        } elseif ($step == 4) {
            $data = [
    ...
    private function buildData($salechannel = array(), $step) {
        ...
        if (is_array($salechannel) && count($salechannel)) {
            ...
            if ($step == 1) {
                ...
                $sql = 'SELECT sc.travelProductId,  cp.notice, ct1.hotline, sp.creator, sp.useEndedAt, sp.usageNotice, sp.instantBooking
                                FROM t_bridge.tb_salechannel sc,   tb_cancelpolicy cp, tb_contacts1 ct1, tb_sellerproduct sp
                                WHERE sc.sellerProductId = cp.sellerProductId  AND sc.sellerProductId = ct1.sellerProductId 
                                    AND sc.sellerProductId = sp.sellerProductId AND sc.travelProductId = "' . $goodIds . '"';

                $this->_model->setQuery($sql);
                $results5 = $this->_model->readAll();
                if ($results5) {
                    $data = [
                        "basicPrice" => "상세 정보 참조",
                        "policy" => [
                            "cancellation" => "1.단순변심, 착오구매 등에 따른 청약철회는 재화 구매 계약 체결에 대한 수신확인 통지 받은 날로부터 7일 이내 가능하며 반품비용은 소비자가 부담함
                                               2.재화 등의 내용이 표시·광고의 내용과 다르거나 계약내용과 다르게 이행된 경우에는 그 재화 등을 공급받은 날부터 3개월 이내, 그 사실을 안 날 또는 알 수 있었던 날부터 30일 이내에 청약철회 등을 할 수 있으며 반품 비용은 판매자가 부담함
                                               3.단, 상품의 특성과 거래조건에 따라 청약철회가 일부 제한될 수 있으며 청약철회에 소요되는 비용이 다르게 책정될 수 있음(딜 상세페이지 참조)
                            ...
                            "complaints" => "1.전자상거래 등에서의 소비자보호에 관한 법률 및 관련규정에 부합하는 기준에 따름
                                             2.마이페이지 내 1:1문의를 통해 문의해주세요.",
                            "conditions" => "사이트 하단의 위메프 이용약관 참조",
                            "delivery" => "재화 등의 공급방법 및 공급시기를 입력해주세요.",
                            "refunds" => "1.재화 등의 교환∙반품∙보증 조건 및 품질보증기준​- 전자상거래 등에서의 소비자보호에 관한 법률 및 관련 규정에 부합하는 기준에 따름​- 일부 상품의 경우 구체적인 교환∙반품∙보증 기준을 딜 상세페이지에 안내
​                                              2.주문취소 및 결제대금 환불 신청 방법​- 배송상품: 주문취소 및 결제대금의 환불은 마이페이지 내 신청 및 확인 가능​- 그 외 상품: 마이페이지>1:1문의를 통해 문의​
                                          3.환불의 지연에 따른 배상금 지급의 조건 및 절차​-전자상거래 등에서의 소비자 보호에 관한 법률 규정에 따라 재화 등을 반환 받은 날로부터​3 영업일 이내에 지급받은 대금의 환급을 정당한 사유 없이 지연하는 경우 연 15%의 ​지연배상금 청구 가능"
                        ],
                        "product" => [
                            "agency" => !empty($results5[0]['creator']) ? $results5[0]['creator'] : "Test",
                            "period" => !empty($results5[0]['useEndedAt']) ? $results5[0]['useEndedAt'] : "Test",
                            "store" => ($results5[0]['instantBooking'] == 0) ? 'NO' : 'YES',
                            "refund" => !empty($results5[0]['notice']) ? $results5[0]['notice'] : "Test",
                            "contactUs" => !empty($results5[0]['hotline']) ? $results5[0]['hotline'] : "Test"
                        ]
                    ];
                }
            }//end $results5
    ...
    public function autoupdateProductWMPAction() {
        $token = $this->getTokenLogin();        
        $sql = 'SELECT * FROM tb_salechannel sc WHERE sc.channelId=2 AND sc.isDelete=0 AND sc.`statusedit`=1';
        $this->_model->setQuery($sql);
        $salechannels = $this->_model->readAll();
        if ($salechannels) {
            foreach ($salechannels as $sc) {
                
                if ($sc['travelProductId'] != "" && $sc['statusedit'] == 1) {
                    //update lên WMP                    
                    if ($token) {
                        $goodIds = $sc['travelProductId'];
                        for($step=2;$step<6;$step++){
                            $data = $this->buildData($sc, $step);
                            //$result = $this->updateProductWMP($token, $goodIds, $data, $step);
                            $result[$step] = $this->updateProductWMP($token, $goodIds,$data, $step);
                        }
                    ...
                    //chuyen statusedit=2
                    $this->_model->updateRecord('salechannel', ['statusedit' => 2], ['where' => "sellerProductId='" . $sc['sellerProductId'] . "'"]);
                } else {                    
                    //get info product
                    $type_rate=$sc['type_rate'];
                    $sellerProductId = $sc['sellerProductId'];
                    $sql = 'SELECT sp.*,ct1.sellerProductId,ct1.name as name_company,ct1.roadNameAddress,ct1.landNumberAddress,ct1.phonetable,ct1.phoneadvisory,ct1.hotline,ct1.website,ct1.email FROM tb_sellerproduct sp,tb_contacts1 ct1 WHERE sp.sellerProductId=' . $sellerProductId . ' AND sp.sellerProductId=ct1.sellerProductId';
                    $this->_model->setQuery($sql);
                    $product = $this->_model->read();
                    if ($product) {
                        $dataIsert = [
                            'alerts' => [
                                [
                                    'email' => $product['email'],
                                    "id" => $sellerProductId,
                                    "kakaoMobileNum" => "testing Kakao36",
                                    "subject" => $product['hotline'],
                                    "useEmail" => false,
                                    "useKakao" => false
                                ]
                            ],
                            "changeableProductFee" => true,
                            "facility" => [
                                "address1" => $product['roadNameAddress'],
                                "address2" => $product['landNumberAddress'],
                                "latitude" => "33.43333730041634",
                                "longitude" => "126.38019015260834",
                                "mainPhone" => $product['phonetable'],
                                "name" => $product['name'],
                                "subPhone" => $product['phoneadvisory'],
                                "subPhoneName" => $product['hotline'],
                                "url" => $product['website']
                            ],
                            "mdId" => "201712005936",
                            "mdName" => "김희재",
                            "partnerId" => "tbridge1",
                            "partnerName" => $product['name_company'],
                            "productFee" => [
                                "rateType" => "PERCENT",
                                "rateValue" => $type_rate.".00"
                            ],
                            "locationType" => "DOMESTIC"
                        ];
                        $goodIds = $this->insertProductWMP($token, $dataIsert);
                        if($goodIds){
                            $this->_model->updateRecord('salechannel',
                                                        ['statusedit' => 1,'travelProductId' => $goodIds],
                                                        [ 'where' => "id=". $sc['id']]
                                                        );
                        }
                    }
        ...
        // foreach
        // nếu có googids và statuedit =1 thì update > WMP và statusedit = 2
        // nếu ko có goodids thì insert.
    }
    /*
    public function updateProductAction() {
        ...
        $sql = 'SELECT sc.travelProductId 
                FROM t_bridge.tb_salechannel sc, tb_channels c
                WHERE sc.channelId = c.channel_id AND c.channel_cid="WMAKE" AND sc.sellerProductId = ' . $salechannel['sellerProductId'];
        $this->_model->setQuery($sql);
        $salechannels = $this->_model->readAll();

        foreach ($salechannels as $item) {
            if (!empty($item['travelProductId'])) {
                $goodIds = trim($item['travelProductId']);
                switch ($step) {
                    case 3:
                        $data = [
                            "removeCategoryValues" => []
    ...
    public function addProductAction() {
        ...
        if ($token) {
            ...
            $dataIsert = [
                'alerts' => [
                    [
                        'email' => "testingkakaogv@gmail.com36",
                        "id" => 0,
                        "kakaoMobileNum" => "testing Kakao36",
                        "subject" => "093965756636",
                        "useEmail" => false,
                        "useKakao" => false
                    ]
                ],
                ...
                "facility" => [
                    ...
                    "address2" => "testing address detail36",
                    "latitude" => "33.43333730041634",
                    "longitude" => "126.38019015260834",
                    "mainPhone" => "08456611336",
                    "name" => "testing name36",
                    "subPhone" => "11435",
                    "subPhoneName" => "08435",
                    "url" => "http://24h.com.vn36"
                ],
                "mdId" => "201712005936",
                "mdName" => "김희재",
                "partnerId" => "tbridge1",
            ...
            if (isset($goodIds) && $goodIds) {
                ...
                for ($step = 2; $step < 6; $step++) {
                    //$salechannel = $this->_model->loadRecord('salechannel', ['travelProductId' => $goodIds , 'channelId' => 2]);
                    //$data=$this->buildData($salechannel, $step);
                    $result[$step] = $this->updateProductWMP($token, $goodIds, $step);
                }
                if (isset($result) && is_array($result)) {
                    echo 'Have ' . count($result) + 1 . ' step action successfull';
                }
    ...
    // Phương thức index
    public function indexAction($StartDate = null, $EndDate = null) { //tour    
        ...
        $url = "https://tour-partner-api.wonders.app/v1/partner/activity/reservations/tickets?used=false&reservationTicketStatuses=B,R,F&ticketSearchDateType=PAID_DATE&startDate=$StartDate&endDate=$EndDate&partnerType=id&bookerType=name&reservationType=booking&goodsIdType=wid&pageSize=30&pageNumber=1&ticketOrderBy=PAID_DATE&locationType=DOMESTIC";
    ...
    // Phương thức index
    public function questionAction($StartDate = null, $EndDate = null) { //question 
        ...
        $token = "Bearer " . $result['token'];
        // lấy tổng số câu hỏi
        $url = "https://tour-partner-api.wonders.app/v1/partner/activity/products/search?saleStatuses=SALE,READY,HOLD,SOLDOUT,STOP,END,TEMP&dateType=CREATED_DATE&startDate=$StartDate&endDate=$EndDate&pageNumber=1&pageSize=30&orderBy=CREATED_DATE&locationType=DOMESTIC";
        $obj = $this->callAPI("GET", $url, null, $token);
        $totalCount = $obj['totalCount'];
        // lấy tất cả nội dung câu hỏi
        $url = "https://tour-partner-api.wonders.app/v1/partner/activity/products/search?saleStatuses=SALE,READY,HOLD,SOLDOUT,STOP,END,TEMP&dateType=CREATED_DATE&startDate=$StartDate&endDate=$EndDate&pageNumber=1&pageSize=$totalCount&orderBy=CREATED_DATE&locationType=DOMESTIC";
        $obj = $this->callAPI("GET", $url, null, $token);
        print_r($obj);
        foreach ($obj['results'] as $item) {
            ...
            foreach ($objQuestion as $itemQuestionAnswer) {
                if ($itemQuestionAnswer) {
                    ...
                    $result = $this->_model->loadRecords('question', ['inquiryId' => $inquiryId,'channel_name'=>'WMAKE']);
                    $data = [
                    ...
                    if ($result) {
                        $this->_model->updateRecord('question', $data, ['inquiryId' => $inquiryId, 'channel_name'=>'WMAKE']);
                    } else {
    ...
    public function replyQuestionAction($inquiryId, $goodIds, $content){
        $url = "https://tour-partner-api.wonders.app/v1/partner/user/login";
        $data = '{"loginType":"PARTNER","userId":"tbridge1","userPwd":"dwkez6nivv"}';
        $result = $this->callAPI("POST", $url, $data, "");
        ...
        $url = "https://tour-partner-api.wonders.app/v1/partner/activity/qna/$inquiryId/answer";
        $data = [
            'answer' => $content,
            'id' => $inquiryId,
            'goodsId' => $goodIds
        ...
        $objQuestion = $this->callAPI("POST", $url, $jsonData, $token);
        echo "<pre>";
        print_r($objQuestion);
