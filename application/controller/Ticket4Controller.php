<?php class Ticket4Controller extends Controller
{
    ...
    public function editticket4Action()
    {
        ...
        $result=$this->_model->loadRecords('contents',['sellerProductId'=>$sellerProductId],true,[ 'sort' => [ 'column' => "seq",'order' => "ASC" ] ]);
        $list_contents=array();
        $format="IMAGE";
        foreach ($result as $value =>$giatri){
    ...
    public function uploadImage10Ajax(){
        ...
        if( isset($_FILES['file10'])){
            $countImageOption = (int)$_POST['countImageOption'];
            $file = new Upload();
            for($i=0; $i<count($_FILES['file10']['name']);$i++){
                ...
                if(!$file->showError()){
                    ...
                    $str=htmlentities('<tr id="tbContent'.$contentId.'" class="contBox" style="padding: 10px">
                			<td>
                			    <input type="hidden" name="seq' . $contentId . '" id="seq' . $contentId . '" value="'. $countImageOption . '"/>
                                    <div class="seq">
                                        <input  type="text" style="width:70px;" disabled="disabled" value="'. $countImageOption . '">
                                        <div id="upOption">
                                            <a onclick="upOption(' . $contentId . ');"><i class="fa fa-angle-up fa-6" title="up" aria-hidden="true" style="font-size:14px;"></i><span class="sr-only">up</span></a>
                                        </div>
                                        <div id="downOption">
                                            <a onclick="downOption(' . $contentId . ');"><i class="fa fa-angle-down" title="down" aria-hidden="true" style="font-size:14px;"></i><span class="sr-only">down</span></a>
                                        </div>
                                    </div>
                            </td>
                			<td>
                			    ...
                			    <div style="display: inline-block; vertical-align: top; padding-top: 5px;">
                                    <a onclick="delContent(' . $contentId . ');" class="btn hover small" style="padding: 5px;" title="Delete"> X </a>
                                </div>
                            </td>
                			<td>
                			    <input type="checkbox" name="checkbox[]" id="checkbox'. $contentId .'" value="'. $contentId .'" class="checkboxOption" />
                			    <input name="contentId[]" id="contentId" type="hidden" value="'.$contentId.'" />
                                <input name="ptitle[]" id="ptitle'.$contentId.'" type="text" value="a" style="display: none;">
                                <select name="" style="width: 150px; height: 24px;display: none;" class="TextBoxField">
                                            <option value="TEXT" >IMAGE</option>
                    ...
                    echo $str . 'count_image_option_m' . $countImageOption;
