<?php 
    class OrderController extends Controller{        
        ...
        public function indexAction(){           
            ...
            if (isset($this->_params['m_search'])) {
                ...
                if($text_ten_phone_mave!=""){
                    $sql .= ' AND (sp.`name` LIKE "%'.$text_ten_phone_mave.'%" OR tp.`phoneNumber` LIKE "%'.$text_ten_phone_mave.'%" OR tp.`ticketNumber` LIKE "%'.$text_ten_phone_mave.'%")';
                }
                //thoigian
                ...
                $text_ten_phone_mave = $this->_params['text_ten_phone_mave']; 
                if($text_ten_phone_mave!=""){
                    $sql .= ' AND (tp.`userName` LIKE "%'.$text_ten_phone_mave.'%" OR tp.`phoneNumber` LIKE "%'.$text_ten_phone_mave.'%" OR tp.`ticketNumber` LIKE "%'.$text_ten_phone_mave.'%")';                  
                }
                $thoigian=0;
                ...
                if (isset($this->_params['method'])){
                    ...
                    if($method=='today'){
                        ...
                        $m_status3=0;
                        $m_status4=0;
                        $m_start = date('Y-m-d', strtotime('-1 year'));
                        $m_end = date("Y-m-d");
        ...
        // Phương thức index
        public function statusTicketAction(){           
            $rows= [];
            $m_channel = array();
            $statusTicket=$this->_params['statusTicket'];
            $text_input_boloc=$this->_view->getItem('language', 'l_boloc'); 
            // Nhà cung cấp
            $list_supplier = $this->_model->loadSupplier(Structure::getUserId());
            $this->_view->setData('list_supplier', $list_supplier);
            ...
            $sql .= ' AND ( tk.`statusTicket`='.$statusTicket.') ';       
            ...
            //echo $sqlheader.$sql; die();
            $items = $this->_model->loadOrder($sqlheader.$sql,$sqlcount.$sql,DEFAULT_LENGTH);            
            $pagination = $this->paginationParams($items['count'], $this->route('orderstatus',['statusTicket'=>$statusTicket]));
            if($pagination){
                $rows = $items['data'];           
                $this->_view->setData('length', $pagination['length']);
                $this->_view->setData('pagination', $pagination['pagination']);
            ...
            $this->_view->setData('ordertbody', Helper::createRowOrderStatusTicket($rows));
            $this->_view->setData('statusTicket', $statusTicket);
            $this->_view->setData('text_input_boloc', $text_input_boloc);
            if($_SESSION['mobile']!=0){
                    $this->_view->setFileTemplate('web','temple_mobile');
                    $this->_view->render('statusticket_mobi');
            }else{
                    $this->_view->render('index');
            }            
