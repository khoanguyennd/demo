...
//             echo ($s2-$s1);
        ...
        public function websrapingAjax(){
            ...
            $wmp->indexAction($StartDate, $EndDate);
            //tmon
            $StartDateDot = date("Y.m.d",strtotime("-1 day "));
            $EndDateDot = date('Y.m.d'); 

            $tm = new TmonController($this->_params);
            $tm->indexAction($StartDateDot, $EndDateDot);

            //gmarket
            $gmk = new GmarketController($this->_params);
            $gmk->indexAction($StartDate, $EndDate);
