...
class TmonController extends Controller {
    ...
    // Phương thức cancel
    public function cancelAction($start_date = null, $end_date = null) {
        set_time_limit(1500);
    ...
    public function questionAction($start_date = null, $end_date = null) {
        $data=[
            ...
            'install_nos'=>'N',
            'ts'=>'1598413535',
            'hash'=>'9a46c859d9ff9dc54c5c1106f0a03a4f6d9ed35a',
            'login_id'=>'',
            'passwd'=>'',
            'secureLogin'=>true,
            'RtnURL'=>'https://ps.tmon.co.kr/daily/'
        ];
        $client = new Client(HttpClient::create(['timeout' => 60]));
        $client->request('POST', 'https://ps.tmon.co.kr/loginCheck',$data);
        ...
        $this->_model->updateRecord('question', ['isDelete' => 1], ['where' => 'question_created > "'.$start_date.' 00:00:00" AND channel_name="TMON"']);  
        $sqlheader = "SELECT sc.travelProductId,sc.channelId";
        $sql = ' FROM  `tb_salechannel` sc,tb_channels cn';
        $sql .= ' WHERE sc.channelId=cn.channel_id  AND cn.channel_id=3 AND sc.travelProductId!=""';
        $list_product = $this->_model->setQuery($sqlheader . $sql);
        ...
        foreach ($list_product as $rows_pro) {
            ...
            $html_string = $client->getResponse()->getContent();
            $pieces = explode('<em class="in">', $html_string);
            $pieces1 = explode('</em>', $pieces[1]);
            $numbePage =ceil($pieces1[0]/20);
           
            if($numbePage==1){
                $this->insertQuestion($dealID, $html_string);
            }else{
                $this->insertQuestion($dealID, $html_string);
                for ($k = 2; $k <= $numbePage; $k++) { // for từng trang
                    $crawler = $client->request('GET', 'https://ps.tmon.co.kr/inquiry/detail?page='.$k.'&main_deal_srl='.$dealID.'&srchCategory=ALL&srchStartDate='.$start_date.'&srchEndDate='.$end_date.'&srchKeyword=&_=1594695300901');
                    $html_string = $client->getResponse()->getContent();
                    $this->insertQuestion($dealID, $html_string);
    ...
    public function insertQuestion($dealID,$html_string){
        $dongs = explode('<td class="deal">', $html_string);
        
        if(isset($dongs[1]))
        for ($i = 1; $i <count($dongs); $i++) { // for từng dòng
            $dong = $dongs[$i];
            
            $isDelete=0;
            $pos=strpos($dong, "사용자의 요청에 의하여 삭제 되었습니다."); // check cau hoi bi xoa
            if($pos!="")
                $isDelete=1;
            $td = explode('</td>', $dong);
            $dealName = $td[0];
            
            $td = explode('<strong class="uid">', $dong);
            $td1 = explode('</strong>', $td[1]);
            $td2=explode('&nbsp;&nbsp;(주문번호 : ', $td1[0]);
            $username= trim($td2[0]);
            $orderNumer="";
            if(isset($td2[1])){
                $orderNumer=substr(trim($td2[1]), 0,-1);
            }
            
            $td = explode('<span class="date">', $dong);
            $td1 = explode('</span>', $td[1]);
            $date_question=$td1[0];
            
            $date_anwer="";
            if(isset($td[2])){
                $td1 = explode('</span>', $td[2]);
                $date_anwer= $td1[0];
            }
            
            $username_anwer="";
            $td = explode('<strong class="pid">', $dong);
            if(isset($td[1])){
                
                $td1 = explode('</strong>', $td[1]);
                $username_anwer= $td1[0];
            }
            
            $td = explode('<p>', $dong);
            $td1 = explode('</p>', $td[1]);
            $question_content=$td1[0];
            ...
            $td = explode('<textarea cols="30" rows="5" name="t_', $dong);
            $td1 = explode('"></textarea>', $td[1]);
            $inquiryId=$td1[0];
            
            $purchase="";
            $td = explode('<span class="buy">', $dong);
            if(isset($td[1])){
                $td1 = explode('</span>', $td[1]);
                $purchase=$td1[0];
            }
            
            $td = explode('<span class="non_buy">', $dong);
            if(isset($td[1])){
                $td1 = explode('</span>', $td[1]);
                $purchase=$td1[0];
            }
            
            $result = $this->_model->loadRecords('question', ['inquiryId' => $inquiryId,'channel_name'=>'TMON']);
            $data = [
                'question_name' => trim($username),
                'question_email' => trim($username),
                'question_content' => trim($question_content),
                'question_created' => trim($date_question),
                'question_status' => 1,
                'orderNumber' => trim($orderNumer),
                'dealId' => $dealID,
                'dealName' => trim($dealName),
                'question_purchase' => trim($purchase),
                'channel_name' => 'TMON',
                'inquiryId' => $inquiryId,
                'isDelete' => $isDelete
            ];
            if ($username_anwer != "") {
                ...
                $data['reply_name'] = trim($username_anwer);
                $data['reply_email'] = trim($username_anwer);
                $data['reply_created'] = trim($date_anwer);
                $data['reply_content'] = trim($anwer_content);
            }
            
            if ($result) {
                $this->_model->updateRecord('question', $data, ['inquiryId' => $inquiryId,'channel_name'=>'TMON']);
            } else {
