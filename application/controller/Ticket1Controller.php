...
//             print_r($account);
            ...
            if($account['role']==0){
                ...
                $sql='  SELECT *
                    FROM tb_user u
                    WHERE u.idx="'.$account['idx'].'"  OR u.idx_parent=(SELECT u1.idx FROM tb_user u1 WHERE u1.idx="'.$account['idx'].'")  ';
            }
            $this->_model->setQuery($sql);
            $list_supplier = $this->_model->readAll();
            $this->_view->setData('list_supplier', $list_supplier);
            $this->_view->setData('onloadsalechannel', $this->onloadsalechannelAjax($sellerProductId,0));
    ...
    public function editticket1Action()
    {
        ...
        if (isset($this->_params['btnSumit']) && $this->_params['btnSumit'] == "editticket1") {
            ...
            $data=['sellerProductId' => $sellerProductId,
                ...
                'displayKeyword' => $displayKeyword,
                'displayCategoryCode' => $displayCategoryCode];
            if(isset($this->_params['type1Ids'])){
                $type1Id =  $this->_params['type1Ids'];
                $type2Id = $this->_params['type2Ids'];
                //             $type3Id = $this->_params['type3Id'];
                //             $type4Id = "";
                ...
                //                 $type4Id = $this->_params['type4Id'];
                //             }
                $data['idType1'] = $type1Id;
                $data['idType2'] = $type2Id;
            }
        ...
        $list_sellerProduct = $this->_model->loadRecords('sellerproduct', [ 'sellerProductId' => $sellerProductId ]);
        ...
        if($account['role']==0){
            ...
            $sql='  SELECT *
                    FROM tb_user u
                    WHERE u.ID="'.$list_sellerProduct[0]['creator'].'"  OR u.idx_parent=(SELECT u1.idx FROM tb_user u1 WHERE u1.ID="'.$list_sellerProduct[0]['creator'].'")  ';
        }
        ...
        $this->_view->setData('list_supplier', $list_supplier);
        $this->_view->setData('onloadsearchtags', $this->onloadsearchtagsAjax());
        $this->_view->setData('onloadsalechannel', $this->onloadsalechannelAjax($sellerProductId,$noedit));
    ...
    public function onloadsalechannelAjax($sellerProductId,$noedit)
    {
    ...
    public function changeChannelAjax()
    {
        ...
        $str = '<select class="select small full" class="TextBoxField" name="type4Id" id="type4Id" >';
    ...
    public function changeCaptchaAjax()
    {
