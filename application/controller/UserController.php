...
class UserController extends Controller {
        ...
        if (isset($this->_params['aid'])) {
            ...
            if ($inputs['aid'] && $inputs['apw']) {
                ...
                if ($login) {
                    ...
                    setcookie("accountremember[ID]", $inputs['aid'], $timeout);
                    setcookie("accountremember[PW]", $inputs['apw'], $timeout);
    ...
    public function registerUserAction() {
        ...
        if (isset($this->_params['name']) && isset($this->_params['phone']) && isset($this->_params['point'])) {
            ...
            if($point>0){
                if($name!=""){
                    if (Func::isName($name) == false) {
                        if (!$this->_model->checkNameExist($this->_params['name'])) {
                            if($phone!=""){
                                if(Func::isPhoneNumric($phone) != false ){
                                    $data = [
                                    ...
                                    if (!$this->_model->checkUserExist($this->_params['phone'])) {
                                        ...
                                        $result['msg'] = $this->_view->getItem('language', 'l_error_unique_phone'); 
                                    }
                                }else{
                                    $result['msg'] = $this->_view->getItem('language', 'l_error_type_phone'); 
                                }
                                
                            }else{
                                $result['msg'] = $this->_view->getItem('language', 'l_error_required_phone'); 
                            }
                        }else{
                            $result['msg'] = $this->_view->getItem('language', 'l_error_unique_name'); 
                        }
                    }else{
                        $result['msg'] = $this->_view->getItem('language', 'l_error_special_name');
                    }               
                }else{
                                ...
                                if(Func::isPhoneNumric($phone) != false ){
                                    ...
                                    if (!$this->_model->checkUserExist($this->_params['phone'])) {
                                        ...
                                        if ($query) {
                                            ...
                                            if ($_SESSION['mobile'] != 0) {      
                                                $result['adduser'] = '<div class="col-3 tv_rows">
                                                                         <a href="" style="color: white">
                                                                        <div class="thanh-vien-item">
                                                                            <div class="thanh-vien-ct">
                                                                                <div  class="name-thanh-vien color-w text-center mg-b-10 w-100 p-0 lh-40">'. $infouser["name"] .'</div>
                                                                                <div class="row mg-b-10">
                                                                                    ...
                                                                                    <div class="col-6">
                                                                                        ...
                                                                                        '.$this->_view->getItem('language', "l_avg").': <span class="color-or">'.$infouser["avg"].'</span>
                                                                                        </div>
                                                                                        <div class="text-cus-hg font-10">
                                                                                        '.$this->_view->getItem('language', "l_detail_high").': <span class="color-or">'. $infouser["highrun"] .'</span>
                                                                                        </div>
                                                                                    </div>
                                                                                    <div class="col-3">
    ...
    public function listMemberAction() {
        ...
        if ($_SESSION['mobile'] != 0) {      
            ...
            $this->_view->render('danhsachtv_mobi');
        } else {
    ...
    public function searchMemberAction() {
        ...
        if (isset($this->_params['keyword'])) {
            ...
            $sql = "SELECT user.* FROM thanhvien AS user
                    WHERE ( user.name LIKE '%" . $keyword . "%' OR  user.phone LIKE '%" . $keyword . "%' ) ";
            
            $settingplayer=0;
            if(isset($this->_params['settingplayer']) && $this->_params['settingplayer']!=0){
                $settingplayer = $this->_params['settingplayer'];
                $sql .= " AND user.id !='" .$settingplayer."' ";
            }
    ...
    public function editlistmemberUserAction(){    
        $result = [
            'status' => false,
            'msg' => 'edit user failt'
        ];

        if (isset($this->_params['id']) && isset($this->_params['name']) && isset($this->_params['point']) && isset($this->_params['phone'])) {
            $id = str_replace(' ', '', $this->_params['id']);
            $name = str_replace(' ', '', $this->_params['name']);
            $point = str_replace(' ', '', $this->_params['point']);
            $point=(int)($point);            
            if($point>0){
                if($name!=""){
                    if (Func::isName($name) == false) {
                        if ($this->_model->checkNameExistForEdit($this->_params['name'], $id) == false) {
                            if($this->_params['phone'] !=""){
                                 if ($this->_model->accuracyPhone($this->_params['phone'], $id)) {
                                    $data = [
                                        'name' => $this->_params['name'],
                                        'diem' => $this->_params['point'],
                                    ];  
                                    $query = $this->_model->updateRecord('thanhvien', ['name'=>$name, "diem"=>$point], ['id'=>$id]);                                   
                                    $result['status'] = true;                   
                                    $result['msg'] = $this->_view->getItem('language', 'l_edit_member_success');
                                }else{
                                    $result['msg'] = $this->_view->getItem('language', 'l_error_accuracy_phone'); 
                                } 
                            }else{
                                $result['msg'] = $this->_view->getItem('language', 'l_phone_c_input');
                            }
    ...
    public function getInfoUserAjax(){
        ...
        if (isset($this->_params['name'])) {
            $name = $this->_params['name'];
            $infouser = $this->_model->loadRecord('thanhvien', ['name' => $name]);
            if($infouser){
                $result['flag'] = true;
                $result['id'] = $infouser['id'];
                $result['diem'] = $infouser['diem'];
