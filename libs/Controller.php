...
class Controller {
	...
	// Biến lưu giá trị $_POS And $_GET
	protected $_params;
	
	// Biến lưu giá trị input text or texteara
	protected $_inputs;
	...
	/* Phương thức thiết lập ngôn ngữ */
	...
	/* Phương thức kiểm tra người dùng đăng nhập
	 * $flag: true không đăng nhập không chuyển trang, đã đăng nhập chuyển trang
	 * $flag: false không đăng nhập được chuyển trang, đã đăng nhập không chuyển trang
	 */
	public function isLogin($flag = false) {	   
	    // Check Cookie
	    if (Session::get ( 'accountshopping' ) == false && isset($_COOKIE['accountshopping']['idx'])){
	        Session::set ( 'accountshopping', $_COOKIE['accountshopping']);
	    }
	    // Check login
	    if (Session::get ( 'accountshopping' ) == false) { // không đăng nhập
	        if($flag === false){
	...
	/* Phương thức lấy giá trị từ form input hoăc texteara và trả về giá trị của mảng
	 * $Option là mảng truyền vào với tên của input
	 * $prefix là tiền tố của bảng trong sql
     */
	...
	/* Phương thức thêm 1 phần tử vào mảng input hoăc texteara*/
	public function appendInput($key, $value) {
		$this->_inputs[$key] = $value;
	}
	
	/* Phương thức trả về mảng từ form input hoăc texteara */
	...
	/* Phương thức kiểm tra giá trị rỗng của phần tử mảng input hoặc texteara */
	...
	/* Phương thức gửi dữ liệu đến một trang */
    ...
    // Phương thức chuyển đổi ngôn ngữ lỗi cho class upload
    ...
    // Phương thức trả về tham số pagination
