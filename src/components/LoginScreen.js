...
import { ScrollView, View, StyleSheet, TextInput, Button, Text, TouchableOpacity, AsyncStorage, CheckBox, Image } from 'react-native';
...
//import SharedStorage from 'react-native-android-toast';
...
//        );
        // NativeModules.SharedStorage.set('This is data from the React Native app');
       //ToastModule.show('Awesome');
       AsyncStorage.getItem('accountshopping', (err, result) => {
          //console.log('remenberAccount: ', result);
          if (result)
            if (JSON.parse(result).rememberAccount) this.props.navigation.navigate('Main');
       });
    }
     ...
     .then((responseJson) => {
        if (responseJson.result) {
            ...
            let UID = {
                 ...
                 rememberAccount: this.state.ischecked
            };
      ...
      return (
        <ScrollView contentContainerStyle={styles.container}>
           ...
           />
            ...
            <View style= {{ width: 300, height : 20 , marginTop: 10 }}>
                <View style={{flex: 1, flexDirection: 'row'}}>
                    <CheckBox
                      style={{top: -4, left: -4}}
                      value={this.state.ischecked ? true : false}
             ...
             <View style= {{ alignItems: 'center', color: '#CCC', width: 300, height : 20 , marginTop: 10 }}>
                <View style={{flex: 1, flexDirection: 'row'}}>
                    <View style={{width: '75%'}} >
                        <Text style={{fontSize: 10}}>아이디/비밀번호 찾기는 PC버전을 이용해주세요</Text>
                    </View>
