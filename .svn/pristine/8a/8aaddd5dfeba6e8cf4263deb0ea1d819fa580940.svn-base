...
class DashboardController extends Controller {
    ...
    public function setting2Action() {
        if ($_SESSION['mobile'] != 0) {      
            $this->_view->setFileTemplate('web', 'temple_mobile');
            $this->_view->render('setting2_mobi');
        } else {
    ...
    public function loaduserAjax() {
        ...
        if (isset($this->_params['settingplayer'])) {
            ...
            foreach ($result as $key => $value) {
                if ($_SESSION['mobile'] != 0) { 
                    $stringdata .= '<div class="col-4 tv_rows" style="padding-left: 2px; padding-right: 2px; margin-bottom:4px" onclick="getPlayer(' . "'" . $value["id"] . "'" . ',' . "'" . $value["name"] . "'" . ',' . "'" . $value["diem"] . "'" . ',' . "'" . $value["avg"] . "'" . ',' . "'" . $value["highrun"] . "'" . ')">
