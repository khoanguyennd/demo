...
class COUPANGApi{
    ...
    public function setParamsApi( $method, $path, $mode){
        ...
        if(is_bool($mode) && $mode==true){
            $this->_header[] = 'Request-Vendor-Id:'.$this->_vendorid;
        }
        return $this;
    }
    
    // Thực hiện xử lý Api; $content là mảng array
    public function connectApi($method,$path,$mode,$content = []){
    ...
    public function createProductContent($sellerProductId,$channelTypeId, $onsale = null){
        ...
        if($productItem){           
            ...
            //.' AND (contactType="WEBSITE" OR contactType="PHONE") ORDER BY contactType DESC '
...
//     							]
    ...
    public function useMultiTicket($ticketNumbers){
...
//         $quantity="1";
    ...
    public function getDetailMultiTicket(){
        $ticketNumber='7000000000151';
        $dealId="3000000001";
        $path1 = '/v2/providers/travel_backbone_api/apis/ticket-api';
