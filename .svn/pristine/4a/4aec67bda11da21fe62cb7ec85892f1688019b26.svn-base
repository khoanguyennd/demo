    ...
    $(document).ready(function () {
        ...
        $('#question').on('click', '.manager', function () {
            ...
            if(channel=="TMON")
                ...
                link_url="http://www.tmon.co.kr/deal/"+dealid;
            if(channel=="COUPANG")
            	//window.open("https://trip.coupang.com/tp/products/"+dealid);
                link_url="https://trip.coupang.com/tp/products/"+dealid;
            if(channel=="WMAKE")
            	//window.open("https://tour.wd.wemakeprice.com/activity/direct/detail/"+dealid);
                link_url="https://tour.wd.wemakeprice.com/activity/direct/detail/"+dealid;
            if(channel=="GMKT")
            	//window.open("http://item.gmarket.co.kr/Item?goodscode="+dealid);
                link_url="http://item.gmarket.co.kr/Item?goodscode="+dealid;
            if(channel=="AUCTION")
            	//window.open("http://itempage3.auction.co.kr/detailview.aspx?ItemNo="+dealid);
                link_url="http://itempage3.auction.co.kr/detailview.aspx?ItemNo="+dealid;
            if(channel=="11ST")
            	//window.open("http://www.11st.co.kr/products/"+dealid);
                link_url="http://www.11st.co.kr/products/"+dealid;
             if ($.isNumeric(qsid)) {
                 $.fn_ajax('viewform', {'qsid': qsid,'link_url': link_url}, function (result) {                    
                     if (result.flag == true) {
                         $.fn_popup(true, 'question');
                         $($.elt_popup).find('.question .content p a.one').attr('href', result.link_url).text(result.qsproduct);
                         $($.elt_popup).find('.question .content p a.two').attr('href', result.link_url);
        ...
        $($.elt_popup + ' .question form').on('submit', function () {
            ...
            if (messagereply && qsid) {
                ...
                $.fn_ajax('excuteform', {'qsid': qsid, 'messagereply': messagereply}, function (result) {
                    if (result.flag == true) {
                        var element = $('#question .manager[data-qsid="' + qsid + '"]').val('답변수정').closest('tr');
...
<div class="t_cauhoi" id="question">
    <div style="position: fixed;background: #fff;z-index: 5;width: 100%; top: 0px;">
        ...
        <div class="t_menu_cauhoi">
            <ul>
                <li class="t_active"><a href="<?= $this->route('question', ['method' => 'unreply']) ?>"><?= $this->language("l_toanbo") ?></a></li>
                <?php foreach ($list_channel as $value => $giatri) { ?>
                    <li><a href="<?= $this->route('questionchannel') ?>/<?= $giatri[1] ?>"><?= $giatri[1] ?></a></li>
                <?php } ?>
            </ul>
        </div>
    </div>
    <div class="t_cauhoi_count question" style="margin-top: 79px;">
        <div class="form_table form_group">
