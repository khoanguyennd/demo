    ...
    static function createRowQuestion($data, $length, $begin){
        ...
        if($data){
            foreach ($data as $key => $value){
                ...
                $strHtml .= '<tr>
                                <td>'.($begin).'</td>
                                <td><input type="button" name="action" value="답변보기" data-qsid="'.$value['question_id'].'" class="btn hover active manager"></td>
                                <td style="text-align:left">'.$statusReply.'</td>
                                <td style="text-align:left">'.$value['channel_name'].'</td>
                                <td style="text-align:left">CP0'.$value['question_id'].'</td>
                                <td style="text-align:left">'.self::catchuoi($value['question_content'],20).'</td>
        ...
        if($data){
            foreach ($data as $key => $value){                
                ...
                $strHtml .= '<tr data-id="'.$value['ticketNumber'].'" data-status="'.$value['statusTicket'].'" data-restore="'.$value['restoreTicket'].'">
								...
								<td>'.$optionId.'</td>
								<td><a href="'.$href.'" target="_blank">'.$value['productname'].'</a></td>
