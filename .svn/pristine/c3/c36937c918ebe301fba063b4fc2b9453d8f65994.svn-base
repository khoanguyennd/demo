<?php $sellerProductId = $this->getData('sellerProductId');
...
$list_type1 = $this->getData('list_type1');
$list_type2 = $this->getData('list_type2');
$list_type3 = $this->getData('list_type3');
$list_type4 = $this->getData('list_type4');
$_SESSION['list_type1'] = $list_type1;
$_SESSION['list_type2'] = $list_type2;
$_SESSION['list_type3'] = $list_type3;
$_SESSION['list_type4'] = $list_type4;
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
     ...
     // Tiếp tục | Trở lại
    	...
    	$($.elt_popup).on('click', '.ticketconfirm .cancel', function(){		
    		$.fn_ticketconfirm(false);
//     		if(jsData.control == 'ticket1' && jsData.action == 'addticket'){
//				$.fn_alert(true, true, '<?=$this->language('l_alertticket')?>');
// 				return false;
// 			}
    	});
...
//             });    
        ...
        // check useEndedAt input date
        $('#useEndedAt').on('change',function(){
           var useStartedAt = $('#useStartedAt').val();
           var useEndedAt = $('#useEndedAt').val();

           if (useStartedAt > useEndedAt) {
            $.fn_alert(true, true, "<?=$this->language( 'l_warnuseenddate')?>");
            $('#useEndedAt').val("<?= $useEndedAt ?>");			
				return false;
           }

        });

        // check useStartdAt input date
        $('#useStartedAt').on('change',function(){
           var useStartedAt = $('#useStartedAt').val();
           var useEndedAt = $('#useEndedAt').val();

           if (useStartedAt > useEndedAt) {
            $.fn_alert(true, true, "<?=$this->language( 'l_warnendusestartdate')?>");	
            $('#useStartedAt').val("<?= $useStartedAt ?>");		
				return false;
           }

        });

        // check usaleEndedAt input date
        $('#saleEndedAt').on('change',function(){
           var saleStartedAt = $('#saleStartedAt').val();
           var saleEndedAt = $('#saleEndedAt').val();

           if (saleStartedAt > saleEndedAt) {
            $.fn_alert(true, true, "<?=$this->language( 'l_warnsaleenddate')?>");	
            $('#saleEndedAt').val("<?= $saleEndedAt ?>");			
				return false;
           }

        });

        // check saletartdAt input date
        $('#saleStartedAt').on('change',function(){
           var saleStartedAt = $('#saleStartedAt').val();
           var saleEndedAt = $('#saleEndedAt').val();

           if (saleStartedAt > saleEndedAt) {
            $.fn_alert(true, true, "<?=$this->language( 'l_warnsalestartdate')?>");
            $('#saleStartedAt').val("<?= $saleStartedAt ?>");					
				return false;
           }
    ...
    function notlimit(source) {
        if (source.checked){
        	document.getElementById("useEndedAt").value = "9999-12-31";
        	document.getElementById("useEndedAt").setAttribute("disabled", "disabled");
        }
        else {
            var today = new Date().toISOString().slice(0, 10)
            document.getElementById("useEndedAt").value = today;
...
<div class="ct_content">
    ...
    <div class="forminput">
        <form name="nf" method="POST" action="<?= $this->route('addticket') ?>" enctype="multipart/form-data">
            <div class="form_group clearfix frrow">
                ...
                <div class="row rmclass" data-action="viewperson" data-perid="14"> 
                    ...
                    <div class="form_group clearfix">
                        ...
                        <div class="form_item col-xs-3">
                            <input type="text" name="sellerProductId" id="sellerProductId"
                                   value="<?=$sellerProductId;?>" readonly="readonly" class="input full"
                                   autocomplete="off">
                            <input name="btnSumit" id="btnSumit" type="hidden" value="addticket"/>
                        </div>
                    ...
                    <div class="form_group clearfix" style="display: none;">
                        ...
                        <div class="form_item col-xs-10" style="text-align: left;">
                            ...
                            foreach ($list_channel as $value => $giatri) {
                                $i++;
                                ?>
                                <div class="divBtn" style="width: 130px; margin: 2px; display: inline-block;">
                                    <input type="checkbox" name="checkchannel[]" value="<?=$giatri['channel_id'] ?>"
                                           id="checkchannel<?=$i?>" onclick="checkchannel(this)"
                                        <?php
					...
					<div class="form_group clearfix">
                        <div class="form_item col-xs-2">
                            <h3><span class="spanred">*</span><?= $this->language('l_typeproduct') ?></h3>
                        </div>
                        <div class="form_item col-xs-1" style="width: 140px;">
                            <select class="select small full"  id="type1Ids" name="type1Ids">
                            	<?php  
                            	$i=0;
                            	foreach ($tbridgetype1 as $value => $giatri) { 
                            	    if($i==0 ) $idType1=$value; 
                            	    $i++;
                            	    ?>
                           		<option value="<?=$value?>" <?php if($idType1==$value) echo 'selected="selected"';?> ><?=$giatri?></option>
                           		<?php }?>
                            </select>
                        </div>
                        <div class="form_item col-xs-1" style="width: 140px;">
                            <select class="select small full changeTypeProduct"  id="type2Ids" name="type2Ids">
                            	<?php  
                            	$i=0;
                            	foreach ($tbridgetype2[$idType1]  as $value => $giatri) { 
                            	    if($i==0 ) $idType2=$value; 
                            	    $i++;
                            	    ?>
                           		<option value="<?=$value?>" <?php if($idType2==$value) echo 'selected="selected"';?> ><?=$giatri?></option>
                           		<?php  } ?>
                           </select>
                        </div>
                    ...
                    <div class="form_group clearfix">
                        ...
                        <div class="form_item col-xs-2">
                            <select class="select small full" class="TextBoxField" name="cityId"
                                ...
                                foreach ($list_city as $value => $giatri) {
                                    ...
                                    $i++;
                                    ?>
                                    <option value="<?= $giatri['id'] ?>"><?= $giatri["name"] ?></option>
                                    <?php
                                }
                                ?>
                            </select>
                        ...
                        <div class="form_item col-xs-2" id="district">
                            <select class="select small full" class="TextBoxField"
                                    name="districtId" onchange="changedistrict(this.value)"
                                    id="districtId">
                                <?php
                                $i = 0;
                                foreach ($list_district as $value => $giatri) {
                                    ...
                                    if ($giatri['id_city'] == $id_city) {
                                        if ($i == 0) {
                                            $id_district = $giatri['id'];
                                        }
                                        $i++;
                                        ?>
                                        <option value="<?= $giatri['id'] ?>">
                                            <?= $giatri['name'] ?>
                                        </option>
                                        <?php
                                    }
                        ...
                        <div class="form_item col-xs-2" id="ward">
                            ...
                            if ($dem > 0) {
                                ?>
                                <select class="select small full" class="TextBoxField" name="wardId">
                                    <?php
                                    $i = 0;
                                    foreach ($list_ward as $value => $giatri) {
                    ...
                    <div class="form_group clearfix">
                        <div class="form_item col-xs-2">
                            <h3><span class="spanred"><span class="spanred">*</span></span><?= $this->language('l_nameproduct') ?></h3>
                        </div>
                        <div class="form_item col-xs-6">
                            <input name="name" maxlength="70" id="name" data-text="<?=$this->language('l_warnname')?>" type="text" value="" class="input full checkMaxLength"/>
                        </div>
                        <div class="form_item col-xs-4 align-left">
                            <p class="height1 checkMaxLengthView">0/70 <?= $this->language('l_warnname') ?></p>
                        </div>
                    </div>
                    <div class="form_group clearfix">
                        <div class="form_item col-xs-2">
                        ...
                        <div class="form_item col-xs-6">
                            <input id="addsearchtags" type="text" value=""
                                   placeholder="<?= $this->language('l_warntagsproduct') ?>"
                                   class="input full"/>
                            <div class="contBox" id="divsearchtags"
                                 style="padding: 10px 0px 5px 10px; text-align: left; min-width: 700px;line-height: 18px;"><?= $this->getData('onloadsearchtags') ?></div>
                        </div>
                    ...
                    <div class="form_group clearfix" style="display: none;">
                        ...
                        <div class="form_item col-xs-6">
                            <input name="displayKeyword" maxlength="70" id="displayKeyword" type="text" value="test"
                                   class="input full" placeholder="<?= $this->language('l_warndisplayKeyword') ?>"/>
                        </div>
                    ...
                    <div class="form_group clearfix">
                        ...
                        <div class="form_item col-xs-7" id="divpricerepresentative"
                             style="text-align: left; padding-top: 10px;">옵션에서 설정한 대표옵션의 가격으로 자동 셋팅됩니다.
                        </div>
                    ...
                    <div class="form_group clearfix">
                        ...
                        <div class="form_item col-xs-7" id="divpricebasic"
                             style="text-align: left; padding-top: 10px;">옵션에서 설정한 대표옵션의기준가격근거값으로 자동 셋팅됩니다.
                        </div>
                        <div class="form_item col-xs-3 align-left">
                ...
                <div class="row rmclass" data-action="viewperson" data-perid="14">
                    <div class="form_group clearfix">
                        ...
                        <div class="form_item col-xs-2">
                            <input name="useStartedAt" id="useStartedAt" type="text"
                                   value="<?=$useStartedAt?>" class="input full" readonly="readonly"/>
                        </div>
                        <div class="form_item col-xs-2">
                            <input name="useEndedAt" id="useEndedAt" type="text"
                                   value="<?=$useEndedAt?>" class="input full" readonly="readonly"/>
                        </div>
                        <div class="form_item col-xs-2" style="padding-top: 5px;text-align: left;">
                            <input type="checkbox" onclick="notlimit(this)" name="checknotlimit" id="checknotlimit" <?php if ($nolimittime == 1) echo 'checked="checked"';?> />
                    ...
                    <div class="form_group clearfix">
                        ...
                        <div class="form_item col-xs-2">
                            <input name="saleStartedAt" id="saleStartedAt" type="text"
                                   value="<?= $saleStartedAt ?>" class="input full" readonly="readonly"/>
                        </div>
                        <div class="form_item col-xs-2">
                            <input name="saleEndedAt" id="saleEndedAt" type="text"
                                   value="<?= $saleEndedAt ?>" class="input full" readonly="readonly"/>
                        </div>
                    ...
                    <div class="form_group clearfix">
                        ...
                        <div class="form_item col-xs-2">
                            <select name="supplier" id="supplier" class="select small full">
                                <?php foreach ($list_supplier as $value => $giatri) { ?>
                                <option <?php if ($supplier == $giatri['idx']) echo 'selected="selected"';?>  value="<?= $giatri['idx'] ?>"><?= $giatri['ID'] ?></option>
                                <?php } ?>
                    ...
                    <div class="form_group clearfix">
                        ...
                        <div class="form_item col-xs-2" style="min-width: 160px;margin-top:8px;text-align: left;">
                            <input type="radio" name="settlement" value="0" <?php if ($settlement == 0)  echo 'checked="checked"'; ?> id="checksettlement0"/> 
                            <label for="checksettlement0">판매가 기준정산</label>
                        </div>
                        <div class="form_item col-xs-2" style="min-width: 160px;margin-top:8px;text-align: left;">
                            <input type="radio" name="settlement" value="1" <?php if ($settlement == 1) echo 'checked="checked"'; ?> id="checksettlement1"> 
