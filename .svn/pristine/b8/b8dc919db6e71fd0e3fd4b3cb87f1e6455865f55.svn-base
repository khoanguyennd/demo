...
class RevenueController extends Controller
{
    ...
    public function downloadDetailExcelAjax(){
        $format = Func::config('testExcel');
        if($format){
           $sid=44;
            $items = $this->_model->loadSettlementUser($sid);
            //var_dump($items);
            //$items = $this->_model->loadSettlementUserDetail($sid);
             if($items){
                $format['export']['titleName'] = "aaa";
                $format['export']['description'] = "bbb";
                $excel = new OfficeExcel();
                $excel->writeSellertment("testExcel",$items['data']);
               }
          }
    }
    ...
    public function settlementAction()
    {
        ...
        $sql=' FROM `tb_user` u, `tb_channels` c,  `tb_settlementchannel` s 
               WHERE c.channel_id=s.channelId AND s.supplier=u.idx ';
