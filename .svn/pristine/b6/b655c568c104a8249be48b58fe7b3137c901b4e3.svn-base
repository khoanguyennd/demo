...
//         $saleEndedAt= date("Y-m-t");
    ...
    public function addTravelItem4($data)
    {
        $str = '';
        $rateTypes = [
            'BASIC', 'ADULT_DAEIN_AND_CHILD_SOIN', 'ADULT', 'ADULT_DAEIN', 'UNIVERSITY_STUDENT', 'HIGH_SCHOOL_STUDENT',
                ...
                $str .= '<tr id="tbTravelItem' . $sellerTravelItemId . '">
                            ...
                            <td>
                                <select name="onSale' . $sellerTravelItemId . '" onchange="flagChanges();" style="min-width: 75px;width: 75px;padding: 3px 0px 5px 0px;" class="selectonSale">
                                        <option ' . (($onSale  == "Y")  ? 'selected="selected"' : '') . ' value="1">'.$this->_view->getItem('language', 'l_status3').'</option>
                                        <option ' . (($onSale  == "N")  ? 'selected="selected"' : '') . ' value="0">'.$this->_view->getItem('language', 'l_status_1').'</option>
                                </select>
                            </td>

                            <td><input type="text" name="useStartedAt' . $sellerTravelItemId . '" id="useStartedAt' . $sellerTravelItemId . '" value="' . $this->formatDayFileUpload($value['key11']) . '" class="useStartedAt" readonly="readonly" style="width: 80px;" /></td>
                            <td><input type="text" name="useEndedAt' . $sellerTravelItemId . '" id="useEndedAt' . $sellerTravelItemId . '" value="' . $this->formatDayFileUpload($value['key12']) . '" class="useEndedAt" readonly="readonly" style="width: 80px;" /></td>
                            <td><input type="text" name="saleStartedAt' . $sellerTravelItemId . '" id="saleStartedAt' . $sellerTravelItemId . '" value="' . $this->formatDayFileUpload($value['key13']) . '" class="saleStartedAt" readonly="readonly" style="width: 80px;" /></td>
                            <td><input type="text" name="saleEndedAt' . $sellerTravelItemId . '" id="saleEndedAt' . $sellerTravelItemId . '" value="' . $this->formatDayFileUpload($value['key14']) . '" class="saleEndedAt" readonly="readonly" style="width: 80px;" /></td>
...
//upload ticket2
    public function uploadFileExcel1Ajax(){   
        ...
        if(isset($_FILES['fileupload']) && ($_FILES['fileupload']['error'] == 0)){
            ...
            if($dataExcel['success'] == true){
                $result['flag'] = $dataExcel['success'];
                $result['datafile'] =  $this->addTravelItem4($dataExcel['data']);
