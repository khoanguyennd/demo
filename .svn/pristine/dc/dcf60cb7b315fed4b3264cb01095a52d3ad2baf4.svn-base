    	...
    	if($items){
    		foreach ($items as $key => $value){
    			...
    			if($value['status'] == 2 || $value['status'] == 3 || $value['status'] == 4){    				
                    ...
                    if($value['status'] == 2){
                        $status="start";
    				    $reponsive = $this->_Api->changestatusProduct($travelProductId,$status);                                    
    				}                                
    ...
    public function testingAction(){
        if($this->_params['method']){
            ...
            if($method=='insertProduct'){ // Thêm sản phẩm
                //echo $this->_params['apiid1'];
                $reposive = $this->_Api->insertProduct($this->_params['apiid1'],$this->_params['apiid2']);
            }
            ...
            if($method=='statusproductstop'){ // status sản phẩm stop
                $reposive = $this->_Api->changestatusProduct($this->_params['apiid1'],"stop");                 
            }
            if($method=='statusproductstart'){ // status sản phẩm start
                $reposive = $this->_Api->changestatusProduct($this->_params['apiid1'],"start");                 
            }
            if($method=='deleteproduct'){ // xóa sản phẩm
                $reposive = $this->_Api->deleteProduct($this->_params['apiid1']);
            }
            if($method=='addoption'){ // Thêm option
                //echo $this->_params['apiid1']; 
            	$reposive = $this->_Api->insertOption($this->_params['apiid1']);
                //$reposive = $this->_Api->insertProductTravelItem($this->_params['apiid1']);
            }
            if($method=='editoption'){ // Edit option
            ...
            if($method=='checkOrdertravelProductId'){ // Get InventoryList
                $reposive = $this->_Api->checkOrdertravelProductId( $this->_params['apiid1']);
            }
            ...
            if($method=='useMultiTicket'){ // use multi Ticket process
                $reposive = $this->_Api->useMultiTicket();
            }
            ...
            if($method=='getInfoBuyerTicket'){ // 
                ...
                $reposive = json_decode(json_encode($reposive),true);                
                if($reposive['totalCount']>0){
...
//                        echo '</pre>';
            ...
            if($method=='getDetailMultiTicket'){ //
                $reposive = $this->_Api->getDetailMultiTicket($this->_params['apiid1']);
            }
            /////////////////////////////////////////////////////////////////////////////////////////////////////
